...
int main()
{
	...
	auto meshes_bundle = assetManager->getAsset("../../media/color_space_test/R8G8B8A8_RLE.tga", lp);
	assert(!meshes_bundle.isEmpty());
	auto mesh = meshes_bundle.getContents().first[0];
	auto mesh_raw = static_cast<asset::ICPUImage*>(mesh.get());

	asset::IAssetWriter::SAssetWriteParams jazda(mesh.get(), asset::EWF_BINARY);
	assetManager->writeAsset("TEST.tga", jazda);
	}
