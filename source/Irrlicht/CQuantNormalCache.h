...
{ 
public:
	static uint32_t quantizeNormal2_10_10_10(const core::vectorSIMDf& normal);
	static uint32_t quantizeNormal8_8_8(const core::vectorSIMDf& normal);
	static uint64_t quantizeNormal16_16_16(const core::vectorSIMDf& normal);

	static void insertIntoCache2_10_10_10(const core::vectorSIMDf& key, const uint32_t quantizedNormal);
	static void insertIntoCache8_8_8(const core::vectorSIMDf& key, const uint32_t quantizedNormal);
	static void insertIntoCache16_16_16(const core::vectorSIMDf& key, const uint32_t quantizedNormal);

private:
	static inline VectorUV mapToBarycentric(const core::vectorSIMDf& vec)
	{
	...
	template <typename T>
	static inline T restoreSign(const core::vectorSIMDu32& vec, const core::vectorSIMDu32& xorflag, const core::vector4db_SIMD& negativeMask, const uint32_t quantizationBits) 
	{
	...
	static core::vectorSIMDf findBestFit(const uint32_t& bits, const core::vectorSIMDf& normal);

private:
	static core::unordered_map<VectorUV, uint32_t, QuantNormalHash, QuantNormalEqualTo> normalCacheFor2_10_10_10Quant;
	static core::unordered_map<VectorUV, Vector8u, QuantNormalHash, QuantNormalEqualTo> normalCacheFor8_8_8Quant;
	static core::unordered_map<VectorUV, Vector16u, QuantNormalHash, QuantNormalEqualTo> normalCacheFor16_16_16Quant;
};
