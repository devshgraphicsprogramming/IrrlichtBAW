...
**/
    ...
    protected:
		...
		//! @brief Info about vertex attributes (mapped buffers, offsets, strides, divisors)
        IMeshDataFormatDesc<T>* meshLayout;
	    //indices
	    video::E_INDEX_TYPE indexType;
		//! @brief See getBaseVertex() description
	    int32_t baseVertex;
        uint64_t indexCount;
...
*/
		...
		//! @brief Get so called base vertex

		//! Base vertex is an index of vertex to which pointer will be returned when queried for attribute ptr.
		//! Used by ICPUMeshBuffer::getAttribPointer(const E_VERTEX_ATTRIBUTE_ID&) and ICPUMeshBuffer::getAttribute(core::vectorSIMDf&, const E_VERTEX_ATTRIBUTE_ID&, size_t) .
		inline const int32_t& getBaseVertex() const {return baseVertex;}
		//! @brief Set base vertex. See detailed description of getBaseVertex() for more
	...
	{
	    //! @brief vertex position attribute id
		
		//! By default se to EVAI_ATTR0
	    E_VERTEX_ATTRIBUTE_ID posAttrId;

		//! @brief texture coordinate attribute id
		
		//! By deafult unusable (set to EVAI_COUNT)
		E_VERTEX_ATTRIBUTE_ID tcoordAttrId;

		//! @brief normal attribute id
		
		//! By deafult unusable (set to EVAI_COUNT)
		E_VERTEX_ATTRIBUTE_ID normalAttrId;
	public:
	    ICPUMeshBuffer(core::LeakDebugger* dbgr=NULL) : IMeshBuffer<core::ICPUBuffer>(NULL,dbgr), posAttrId(EVAI_ATTR0), tcoordAttrId(EVAI_COUNT), normalAttrId(EVAI_COUNT) {}
		...
		inline const E_VERTEX_ATTRIBUTE_ID& getTcoordAttributeIx() const { return tcoordAttrId; }
		inline void setTcoordAttributeIx(const E_VERTEX_ATTRIBUTE_ID& attrId)
		{
			if (attrId >= EVAI_COUNT)
#ifdef _DEBUG
			{
		...
		virtual core::vectorSIMDf getTcoord(size_t ix) const
		{
			core::vectorSIMDf outPos(0.f, 0.f);
			bool success = getAttribute(outPos, tcoordAttrId, ix);
#ifdef _DEBUG
			if (!success)
			{
				//os::Printer::log("SOME DEBUG MESSAGE!\n",ELL_ERROR);
		...
		virtual core::vectorSIMDf getNormal(size_t ix) const
		{
			core::vectorSIMDf outPos(0.f, 0.f, 0.f);
			bool success = getAttribute(outPos, normalAttrId, ix);
#ifdef _DEBUG
			if (!success)
			{
				//os::Printer::log("SOME DEBUG MESSAGE!\n",ELL_ERROR);
		...
		//! @brief Returns pointer to n-th index of attribute with attrId ID where n=baseVertex (see getBaseVertex())
		...
		//! Outputs (through output arg.) n-th index of attribute with attrId ID where n = ix + baseVertex (see getBaseVertex())
		/** 
		@param output Vector to which attribute will be assigned
		@param attrId Id of the attribute
		@param ix Index in attribute array
		@returns true when ok or false when `ix` is out of buffer range, conversion from the attribute's type to vectorSIMDf is not implemented or other error occured*/
